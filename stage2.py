...
if __name__ == "__main__":
    ...
    from keras.layers import Conv1D, MaxPooling1D, Embedding, LSTM, Bidirectional, Dropout,GRU,SimpleRNN,GlobalAveragePooling1D
    from keras.models import Model
    from CallBackMyForStage2 import CallBackMyForStage2
    import copy
    ...
    capsule = Capsule(num_capsule=4, dim_capsule=32, routings=Routings,
    ...
    model.compile(
        ...
        optimizer='adam',
        metrics=['accuracy'])
    train_label_cat = np_utils.to_categorical(train_label, 4)
    test_backup = np.array(copy.deepcopy(test_label))
    test_label_cat = np_utils.to_categorical(test_label,4)
    callbackmy = CallBackMyForStage2(test_array=test_array,
                            testpos1=test_postion1,
                            testpos2=test_postion2,
                            test_backup=test_backup,
                            filename="log//process//stage2.txt",
                            filename2="log//result//stage2.txt")
    filepath = "log//stage2BestModel-{epoch:03d}-{micro_F1_val:.4f}.hdf5"
    ...
    model.fit([train_array, train_postion1, train_postion2], train_label_cat,
              ...
              callbacks=[callbackmy, checkpoint], verbose=2)
